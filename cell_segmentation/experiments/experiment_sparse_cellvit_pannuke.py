# Sparse CellVit Experiment Class
#
...
class ExperimentSparseCellViTPanNuke(BaseExperiment):
    ...
    def run_experiment(self) -> tuple[Path, dict, nn.Module, dict]:
        ...
        model = self.get_train_model(
            ...
            attention_type=self.run_conf["model"].get("attention_type", "mixed"),
            window_size=self.run_conf["model"].get("window_size", 16),
            top_k_ratio=self.run_conf["model"].get("top_k_ratio", 0.2),
            local_head_ratio=self.run_conf["model"].get("local_head_ratio", 0.6),
        )
        model.to(device)
    ...
    def get_train_model(self, **kwargs) -> SparseCellViT:
        """Return the Sparse CellViT training model"""
        # 从kwargs中提取所需参数，提供默认值
        pretrained_encoder = kwargs.get('pretrained_encoder', None)
        pretrained_model = kwargs.get('pretrained_model', None)
        backbone_type = kwargs.get('backbone_type', 'default')
        shared_decoders = kwargs.get('shared_decoders', False)
        regression_loss = kwargs.get('regression_loss', False)
        attention_type = kwargs.get('attention_type', 'mixed')
        window_size = kwargs.get('window_size', 16)
        top_k_ratio = kwargs.get('top_k_ratio', 0.2)
        local_head_ratio = kwargs.get('local_head_ratio', 0.6)
        ...
        if backbone_type.lower() == "default":
            ...
            if pretrained_model is not None:
                try:
                    ...
                    # 检查权重文件格式
                    model_weights = self._extract_model_weights(cellvit_pretrained)
        ...
        if backbone_type.lower() == "sparse_vit256":
            ...
            if pretrained_encoder is not None:
                try:
                    ...
                    # 检查权重文件格式
                    model_weights = self._extract_model_weights(encoder_weights)
                    
                    # 尝试加载权重
                    try:
                        if hasattr(model, 'load_pretrained_encoder'):
                            self.logger.info("Using model's specific load_pretrained_encoder method")
                            model.load_pretrained_encoder(model.model256_path)
                        else:
                            # 直接加载权重，设置strict=False
                            self.logger.info("Loading weights directly with strict=False")
                            incompatible_keys = model.load_state_dict(model_weights, strict=False)
                            self.logger.info(f"Incompatible keys: {incompatible_keys}")
                        self.logger.info("Loaded pretrained encoder")
                    except Exception as inner_e:
                        self.logger.warning(f"Failed to load encoder weights directly: {inner_e}")
                        # 尝试加载student或teacher部分
                        if isinstance(encoder_weights, dict):
                            if 'student' in encoder_weights:
                                self.logger.info('Trying to use student weights from DINO format')
                                try:
                                    student_weights = encoder_weights['student']
                                    if isinstance(student_weights, dict):
                                        incompatible_keys = model.load_state_dict(student_weights, strict=False)
                                        self.logger.info(f"Loaded student weights. Incompatible keys: {incompatible_keys}")
                                except Exception as student_e:
                                    self.logger.warning(f"Failed to load student weights: {student_e}")
                            elif 'teacher' in encoder_weights:
                                self.logger.info('Trying to use teacher weights from DINO format')
                                try:
                                    teacher_weights = encoder_weights['teacher']
                                    if isinstance(teacher_weights, dict):
                                        incompatible_keys = model.load_state_dict(teacher_weights, strict=False)
                                        self.logger.info(f"Loaded teacher weights. Incompatible keys: {incompatible_keys}")
                                except Exception as teacher_e:
                                    self.logger.warning(f"Failed to load teacher weights: {teacher_e}")
                except Exception as e:
            ...
            if pretrained_model is not None:
                try:
                    ...
                    # 设置strict=False以忽略不匹配的键
                    incompatible_keys = model.load_state_dict(model_weights, strict=False)
                    self.logger.info(f"Loaded model weights. Incompatible keys: {incompatible_keys}")
                except Exception as e:
                    self.logger.warning(f"Failed to load pretrained model: {e}")
    ...
    def _extract_model_weights(self, weights):
        """提取权重字典中的模型权重，支持多种格式"""
        if isinstance(weights, dict):
            if 'model_state_dict' in weights:
                self.logger.info('Using weights from model_state_dict key')
                return weights['model_state_dict']
            elif 'model' in weights:
                self.logger.info('Using weights from model key')
                return weights['model']
            elif 'state_dict' in weights:
                self.logger.info('Using weights from state_dict key')
                return weights['state_dict']
            else:
                self.logger.info('Using weights directly as state_dict')
                return weights
        else:
            self.logger.info('Weights are not a dictionary, using as-is')
            return weights
    ...
    def get_train_model(config):
        if config.model.backbone == 'default':
            model = SparseCellViT(
                ...
                top_k_ratio=config.model.top_k_ratio,
                local_head_ratio=config.model.local_head_ratio,
            )
        elif config.model.backbone == 'sparse_vit256':
            model = SparseCellViT256(
                image_size=config.model.image_size,
                num_classes=config.model.num_classes,
                dropout=config.model.dropout,
        ...
        # 加载预训练编码器权重
        if hasattr(config.model, 'pretrained_encoder') and config.model.pretrained_encoder is not None:
            try:
                # 加载权重文件
                weights = torch.load(config.model.pretrained_encoder, map_location='cpu')
                ...
                if isinstance(weights, dict):
                    ...
                    elif 'student' in weights and isinstance(weights['student'], dict):
                        # 这是DINO格式的权重，使用student部分
                        model_weights = weights['student']
                        self.logger.info('使用DINO格式权重的student部分')
                    elif 'teacher' in weights and isinstance(weights['teacher'], dict):
                        # 如果student不存在，尝试使用teacher
                        model_weights = weights['teacher']
                        self.logger.info('使用DINO格式权重的teacher部分')
                    else:
                        model_weights = weights
                        self.logger.info('直接使用权重字典')
                else:
                    model_weights = weights
                
                # 加载权重，设置strict=False以忽略不匹配的键
                model.load_state_dict(model_weights, strict=False)
                self.logger.info(f'成功加载预训练编码器权重: {config.model.pretrained_encoder}')
            except Exception as e:
                self.logger.warning(f'加载预训练编码器权重失败: {e}')
        
        # 加载预训练模型权重
        if hasattr(config.model, 'pretrained_model') and config.model.pretrained_model is not None:
            try:
                # 加载权重文件
                weights = torch.load(config.model.pretrained_model, map_location='cpu')
                ...
                # 加载权重，设置strict=False以忽略不匹配的键
                model.load_state_dict(model_weights, strict=False)
                self.logger.info(f'成功加载预训练模型权重: {config.model.pretrained_model}')
            except Exception as e:
                self.logger.warning(f'加载预训练模型权重失败: {e}')
        
        # 冻结部分层
        if hasattr(config.model, 'freeze_layers') and config.model.freeze_layers:
            for name, param in model.named_parameters():
                if any(layer in name for layer in config.model.freeze_layers):
                    param.requires_grad = False
            self.logger.info(f'冻结层: {config.model.freeze_layers}')
        
        self.logger.info(f'模型初始化完成，参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad)}')
        return model