# -*- coding: utf-8 -*-
# Test training script for CellViT using small subset of data
#
...
class LimitedPanNukeDataset(PanNukeDataset):
    """有限版 PanNukeDataset，只返回前 N 个样本"""
...
def train(resume_from_checkpoint=False):
    """Training function for CellViT with small dataset subset"""
    # Setup logging
    logdir = Path("./logs/train_cellvit_test")
    logdir.mkdir(exist_ok=True, parents=True)
    
    # Initialize logger first
    logger = Logger(level="INFO", log_dir=logdir, comment="test_training").create_logger()
    logger.info("Starting test training of CellViT with small dataset subset")
    ...
    # Training dataset: PanNuke fold0, only first 100 samples
    train_dataset = LimitedPanNukeDataset(
        limit=100,
    ...
    # Test dataset: PanNuke fold0, only first 50 samples (for validation)
    test_dataset = LimitedPanNukeDataset(
        limit=50,
        dataset_path=pannuke_path,
        folds=[0],
    ...
    scheduler = CosineAnnealingLR(optimizer, T_max=5, eta_min=1e-6)  # 调整为5轮
    
    # Setup early stopping
    early_stopping = EarlyStopping(
        patience=3,  # 早停耐心值，3轮没有提升则停止训练
    ...
    trainer = CellViTTrainer(
        ...
        experiment_config={"epochs": 5, "batch_size": 18},  # 调整为5轮
    ...
    logger.info(f"Starting training for 5 epochs, from epoch {start_epoch+1}")
    
    for epoch in range(start_epoch, 5):  # 调整为5轮
        ...
        train_scalar_metrics, train_image_metrics = trainer.train_epoch(
            ...
            unfreeze_epoch=2  # 在前2个epoch后解冻编码器
        )
        ...
        # Run validation every epoch for test
        val_metric = None
